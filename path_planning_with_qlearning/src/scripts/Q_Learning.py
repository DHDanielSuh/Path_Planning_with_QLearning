...
def spawnModel(modelName, x, y): #  use service to spawn model
	...
	modelPath = os.path.dirname(os.path.dirname(os.path.dirname(os.path.realpath(__file__)))) + "/src/models/turtlebot3_"
...
def genModel(optimizer, loss, lr, first, hidden, last, stateSpace, initializer): # build neural net to be used for DQN
...
if __name__ == '__main__':
	...
	try:
		SESSION_NAME = "Session - " + datetime.datetime.now().strftime("%Y-%m-%d-%H-%M-%S")
		hyperParameterList = { # set hyperparameter combinations
			...
			"Batch Size": [225], # memories examined per step
